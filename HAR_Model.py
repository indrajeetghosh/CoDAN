...
#Please fc5 (output) layer based of the number of classes and also first input layer - based on input shape and also make change here - x = torch.randn(1, 12, 256) - accordingly
...
class HARmodel(nn.Module):
    def __init__(self):
        ...
        self.conv4 = nn.Conv1d(128, 256, 5)
    ...
    def convs(self, x):
        ...
        x = self.maxpool1(self.bn3(F.relu(self.conv4(x))))
    ...
    def forward(self, x):
        ...
        x = self.maxpool1(self.bn2(F.relu(self.conv2(x))))
        x = self.drop(self.maxpool1(self.bn3(F.relu(self.conv3(x)))))
        feature_emb = self.maxpool1(self.bn3(F.relu(self.conv4(x)))) 
